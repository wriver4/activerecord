        ...
        implements iRelations
{
     ...
     * @return void
     */
    protected function query_and_attach_related_models_eagerly(Table $table,
            $models, $attributes, $includes = [], $query_keys = [],
            $model_values_keys = [])
    {
        $values = [];
        ...
        $values = [$values];
        $conditions = SQLBuilder::create_conditions_from_underscored_string($table->conn,
                        $query_key, $values);
        ...
        if (!empty($options['through']))
        {
            ...
            $options['joins'] = $this->construct_inner_join_sql($through_table,
                    true);
        ...
        foreach ($models as $model)
        {
            ...
            foreach ($related_models as $related)
            {
                if ($related->$query_key == $key_to_match)
                {
                    ...
                    {
                        $model->set_relationship_from_eager_load(clone($related),
                                $this->attribute_name);
                    }
                    else
                    {
                        $model->set_relationship_from_eager_load($related,
                                $this->attribute_name);
                    }
            ...
            if (0 === $matches)
            {
                $model->set_relationship_from_eager_load(null,
                        $this->attribute_name);
            }
        }
    }
    ...
    protected function append_record_to_associate(Model $associate,
            Model $record)
    {
    ...
    protected function create_conditions_from_keys(Model $model,
