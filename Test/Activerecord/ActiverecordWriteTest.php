        ...
        extends DatabaseTest
{
    ...
    {
        if (!$this->conn->supports_sequences()) throw new Activerecord\DatabaseException('');

        AuthorWithoutSequence::create(array(
            'name' => 'Bob!'));
    }
    ...
    {
        if ($this->conn->supports_sequences()) $this->assert_equals($this->conn->get_sequence_name('authors',
                            'author_id'), Author::table()->sequence);
        else $this->assert_null(Author::table()->sequence);
    }

    public function test_sequence_was_explicitly_set()
    {
        if ($this->conn->supports_sequences()) $this->assert_equals(AuthorExplicitSequence::$sequence,
    ...
    public function test_delete_by_find_all()
    {
        ...
        foreach ($books as $model) $model->delete();
    ...
    public function test_update_should_quote_keys()
    {
        ...
        $book->name = 'new name';
        $book->save();
        $this->assert_true(strpos($book->connection()->last_query,
                        $book->connection()->quote_name('name')) !== false);
    }
    ...
    public function test_dirty_attributes()
    {
        ...
        $this->assert_equals(array(
            'name',
            'special'), array_keys($book->dirty_attributes()));
    }
    ...
    {
        $book = Book::first();
        $book->set_attributes(array(
            'name' => 'rivers cuomo'));
        $this->assert_equals(array(
            'name'), array_keys($book->dirty_attributes()));
    }
    ...
    {
        $num_affected = Author::delete_all(array(
                    'conditions' => 'parent_author_id = 2'));
    ...
    {
        if (!$this->conn->accepts_limit_and_order_for_update_and_delete()) $this->mark_test_skipped('Only MySQL & Sqlite accept limit/order with UPDATE clause');
        ...
        $this->assert_true(strpos(Author::table()->last_sql,
                        'ORDER BY name asc LIMIT 1') !== false);
    }

    public function test_update_all_with_set_as_string()
    {
        $num_affected = Author::update_all(array(
